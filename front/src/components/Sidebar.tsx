    ...
    <>
      ...
      <div className="flex items-center gap-3 px-4 h-16 border-b border-slate-800/60">
        ...
        {mobileOpen && (
          <button
            onClick={onClose}
            className="ml-auto p-1.5 text-slate-500 hover:text-slate-200 hover:bg-slate-700/50 rounded-lg transition-all lg:hidden"
          >
            <X size={20} />
          </button>
        )}
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Tooltip quando colapsado (apenas desktop) */}
              {collapsed && !mobileOpen && (
    ...
    <>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          {/* Drawer */}
          <aside className="relative w-72 h-full flex flex-col bg-linear-to-b from-slate-900 to-slate-950 border-r border-slate-800/60 shadow-2xl animate-slide-in-left">
            {sidebarContent}
          </aside>
        </div>
      )}
